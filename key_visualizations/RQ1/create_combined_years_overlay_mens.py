...
This script creates two plots (standardized and non-standardized) that combine all men's data
...
def load_data_from_custom_path(data_dir):
    ...
    # Filter to specific distances - men only
...
def create_overlay_plot_from_year_averages(year_data_dict, mode, output_path):
    ...
    for year in years:
        if year in year_data_dict and len(year_data_dict[year]) > 0:
            men_df_year = year_data_dict[year][year_data_dict[year]['gender'] == 'M'].copy()
            all_men_data.append(men_df_year)
    
    if len(all_men_data) > 0:
        all_men_combined = pd.concat(all_men_data, ignore_index=True)
        men_q1 = all_men_combined['first_race_minutes'].quantile(0.25) if len(all_men_combined) > 0 else None
        men_q3 = all_men_combined['first_race_minutes'].quantile(0.75) if len(all_men_combined) > 0 else None
        ...
        men_q1 = men_q3 = men_iqr = men_outlier_threshold = None
    ...
    for year in years:
        ...
        men_df = year_data_dict[year][year_data_dict[year]['gender'] == 'M'].copy()
    ...
    for num_races in race_counts:
        ...
        for minute in minute_values:
            ...
            if key in all_year_bins:
                ...
                # Only include if we have at least 2 out of 3 years (or adjust threshold)
        ...
        if len(averaged_bins) > 1:
            ...
            all_men_minutes.extend(bin_means_df['first_race_minutes'].tolist())
    ...
    if men_q1 is not None and men_q3 is not None:
        ...
        if mode == 'standardized':
            title_text = f'Combined 2023-2025: Standardized (Weather & Elevation) - Male\nIQR: Q1 = {int(round(men_q1))} min, Q3 = {int(round(men_q3))} min'
        else:
            title_text = f'Combined 2023-2025: Converted Only (8000m) - Male\nIQR: Q1 = {int(round(men_q1))} min, Q3 = {int(round(men_q3))} min'
    else:
        if mode == 'standardized':
            title_text = f'Combined 2023-2025: Standardized (Weather & Elevation) - Male'
        else:
...
def main():
    ...
    print("="*60)
    print("CREATING COMBINED RQ1 OVERLAY PLOTS (2023-2025 COMBINED) - MEN'S DATA")
    print("="*60)
    ...
    output_path_conv = os.path.join(output_dir, 'combined_2023_2024_2025_non-standardized_mens.pdf')
    ...
    output_path_std = os.path.join(output_dir, 'combined_2023_2024_2025_standardized_mens.pdf')
    ...
    print(f"  - combined_2023_2024_2025_non-standardized_mens.pdf")
    print(f"  - combined_2023_2024_2025_standardized_mens.pdf")
